...
def ens_match(in_file,tra,tdec,scope='lcogt',out_file='default',d_max=1.0,nn_dist=20,edge_buffer=50, min_apps=30,mag_ulim=50.0,mag_llim=5.0,magerr_ulim=0.5):
    ...
    print('')
    print('Target Distace Match ('+np.str(d_max)+' arcsec = '+np.str(d_max/3600)+')')
    ...
    if cont == 'y':
        print('Continuing with '+np.str(ee.size)+' frames')
        ...
        for i in range(ee.size-min_apps):
                       ...
                       #be used as a comparison star
                        ...
                        if len(matches.keys()) == 1:
                            ...
                            run_ind=np.max(np.array(keys[keys != 'Target'],
                                                    dtype=float))
                            m_name=np.str(np.int(run_ind+1)).zfill(4)
            ...
            print(np.float(i)/(ee.size-min_apps))
...
def ens_lc(in_file,target,date,scope,all_lc='yes',n_min=20,s_width=0.25):
    ...
    def lc_plot(lc_hjd,lc_mag,lc_magerr,lc_p):
        ax.set_title('Lightcurve Plot - '+lc_id[0]+' - '+'N='+
                     np.str(np.sum(lc_p==1))+' - STD = '+np.str(np.round(np.std(lc_mag[lc_p==1]),4)))
    ...
    if redo == 'n':
        ...
        if output == 'y':
            np.savetxt(np.str(target)+'_'+np.str(date)+'_'+np.str(((matches['Target'])[0])[7])+'_enslc_'+np.str(scope)+'.dat',
            ...
            pp = PdfPages(np.str(target)+'_'+np.str(date)+'_'+np.str(((matches['Target'])[0])[7])+'_enshist_'+np.str(scope)+'.pdf')
            ax = plt.subplot(111)
            ...
            pp.close()
            plt.close()
            os.system('mv '+np.str(target)+'_'+np.str(date)+'_'+np.str(((matches['Target'])[0])[7])+'_enshist_'+np.str(scope)+'.pdf ./ens_data/'+np.str(date))

            c_ra=np.empty(len(ss[keys != 'Target']),dtype=np.float64)
            c_ra_std=np.empty(len(ss[keys != 'Target']),dtype=np.float64)
            c_dec=np.empty(len(ss[keys != 'Target']),dtype=np.float64)
            c_dec_std=np.empty(len(ss[keys != 'Target']),dtype=np.float64)
            
            comps=(keys!='Target')

            for i in range(len(ss[comps])):
                ra_i=np.array([row[10] for row in matches[ss[comps][i]]],dtype=np.float64)
                ...
                dec_i=np.array([row[11] for row in matches[ss[comps][i]]],dtype=np.float64)
            ...
            for i in range(len(ss[comps])):
                ...
                np.savetxt(np.str(ss[comps][i])+'_'+np.str(date)+'_'+np.str(((matches['Target'])[0])[7])+'_enslc_'+np.str(scope)+'.dat',
                           np.c_[ss_hjd,ss_ens_mag,ss_ens_mag_err,ss_ss_eff,ss_air],
                           fmt=['%15.10f','%10.5f','%10.5f','%4i','%10.5f'])

                os.system('mv '+np.str(ss[comps][i])+'_'+np.str(date)+'_'+
                          np.str(((matches['Target'])[0])[7])+'_enslc_'+np.str(scope)+
                          '.dat ./ens_data/'+np.str(date))

            f=open(np.str(target)+'_'+np.str(date)+'_'+np.str(((matches['Target'])[0])[7])+\
                       '_enscomps_'+np.str(scope)+'.dat','w')
            f.write('# ID    Mag0    Merr0   RA    '+
                    '          RA STD          DEC             DEC STD       '+
                    '  Mag     Merr    Magstd  \n')
            f.write(np.str(t_ss[0]).zfill(4)+'\t'+
                    "%5.3f" % m0[keys=='Target']+'\t'+
            ...
            for i in range(ss[(p==1) & (keys!='Target')].size):
                f.write(np.str(ss[(p==1) & (keys!='Target')][i]).zfill(4)+'\t'+
            ...
            os.system('mv '+np.str(target)+'_'+np.str(date)+'_'+np.str(((matches['Target'])[0])[7])+\
                          '_enscomps_'+np.str(scope)+'.dat ./ens_data/'+np.str(date))
